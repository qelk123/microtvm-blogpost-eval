...
class OpenOCDLowLevelDevice final : public LowLevelDevice {
 public:
  ...
  explicit OpenOCDLowLevelDevice(const std::string& server_addr, int port) : socket_() {
    ...
    socket_.SendCommand();
    socket_.cmd_builder() << "mww 0xF8000008 0x0000DF0D";
    socket_.SendCommand();
    socket_.cmd_builder() << "mww 0xF8000910 0x1F";
    socket_.SendCommand();
    socket_.cmd_builder() << "mdw 0xF8000910";
    socket_.SendCommand();
  ...
  void Write(TargetPtr addr, const void* buf, size_t num_bytes) override {
    ...
    // Build a command to set the value of `input`.
    {
      ...
      if(addr>=(size_t)heap_address)//((addr-(size_t)0x2001b188)==(std::nullptr_t)0||(addr-(size_t)0x20014d48)==(std::nullptr_t)0||(addr-(size_t)0x20027a08)==(std::nullptr_t)0||(addr-(size_t)0x20008048)==(std::nullptr_t)0||(addr-(size_t)0x2000c048)==(std::nullptr_t)0||(addr-(size_t)0x20007048)==(std::nullptr_t)0)
      {
       for (size_t i = 0; i < num_bytes/4; i++) {
        ...
        cmd_builder << ((static_cast<uint32_t>(char_buf[i])&0xff)+(static_cast<uint32_t>(char_buf[i+1])&0xff)*0x100+(static_cast<uint32_t>(char_buf[i+2])&0xff)*0x10000+(static_cast<uint32_t>(char_buf[i+3])&0xff)*0x1000000)<< " ";
      }
      }
      else
      {
      for (size_t i = 0; i < num_bytes; i++) {
        ...
        // Need to cast to uint, so the number representation of `buf[i]` is
        // printed, and not the ASCII representation.
        cmd_builder << (static_cast<uint32_t>(char_buf[i])&0xff) << " ";
      }
      }
      cmd_builder << "}";
      socket_.SendCommand();
  ...
  void Execute(TargetPtr func_addr, TargetPtr breakpoint_addr) override {
    ...
    /*socket_.cmd_builder() << "zynq.cpu1 arm core_state thumb";
    socket_.SendCommand();

    socket_.cmd_builder() << "zynq.cpu0 arm core_state thumb";
    socket_.SendCommand();*/
