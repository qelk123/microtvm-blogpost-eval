...
void UTVMMain() {
  ...
  // loss of precision should be fine here, since we only care about the lower bits
    if ((uint32_t)utvm_workspace_start&(utvm_word_size-1)){//(((uint32_t)utvm_workspace_start) % utvm_word_size) {//
...
void* TVMBackendAllocWorkspace(int device_type, int device_id, uint64_t size, int dtype_code_hint,
  ...
  size_t tmp=alloc_requested_bytes;
  size_t alloc_size_words =0;
  while(tmp>0)
  {
     tmp=tmp-utvm_word_size;
     alloc_size_words=alloc_size_words+1;
  }
  //size_t alloc_size_words =beishu;//(alloc_requested_bytes + utvm_word_size - 1) / utvm_word_size;
